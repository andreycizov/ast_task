...
class fcreate(Command):
    opcode = 'FCREATE'

    # creates a new fence ID

    def __init__(self, context_name):
        self.context_name = context_name
        super().__init__()

# we've got a fence that is allowed to be locked by several lock IDs
# if they lock that fence - they are allowed to enter it
# for that they need a lock ID

# Firstly, we lock something with N locks
# As soon as all locks are released, we are allowed to proceed
# There, we are allowed for an only single execution path

# create_lock_ids a,b,c == 1, 2, 3
# lock_ids a,b,c == MSET a True b True c True
# create_fence ALL a,b,c == LSET fence_ALL a b c

# exec a
# check_fence_locks_jnz ALL A B
# A: lock_fence_by_id ALL a == SET_IF_NULL fence_ALL_lock a
#    check_fence_locked_jnz ALL A1 B
#    A1: run_fence_exit
# B: exit
...
# for(var('a'), load_accounts(), procedure(var('b'), var('c'), body=[load_account_ftp(var('c'))]))
...
class lpop(Command):
    ...
    # resolve a given lock


class fjnz(Command):
    opcode = 'FJNZ'

    # lock a given fence with a given ID
...
class pop(Command):
    ...
    def __str__(self, *args, **kwargs):
        return '{} {}'.format(super().__str__(*args, **kwargs), ['='.join([a, b]) for a, b in self.kwargs.items()])
